...
from .document_file import (
    ...
    get_base64_size,
    get_max_total_storage_size,
    get_max_upload_size,
    get_max_upload_size_message,
)
...
class DocumentFolder(models.Model):
    ...
    def create_from_upload_tree(self, tree, folder_id):
        ...
        árbol no supere el límite de subida y que sumarla al espacio ya ocupado no supere el
        límite total, para no dejar la carpeta a medio subir si se excede algún límite.
        """
        tree_size = self._get_upload_tree_size(tree)
        if tree_size > get_max_upload_size(self.env):
            raise ValidationError(get_max_upload_size_message(self.env))
        if self.env["document.file"].get_total_storage_size() + tree_size > get_max_total_storage_size(self.env):

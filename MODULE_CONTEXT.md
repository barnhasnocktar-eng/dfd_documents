...
- `default_get` llama a `folder._check_can_manage_self()`: si el usuario no tiene grupo ni empleado permitido en el **padre** de la carpeta (ver más abajo), el wizard ni siquiera abre.
...
- Hay `ir.rule` global (`data/access_permissions.xml`) sobre `document.folder` y `document.file` filtrando por `effective_group_ids` OR `effective_employee_ids.user_id`. **Ojo con la asimetría clave**: `effective_group_ids`/`effective_employee_ids` (lectura/visibilidad, vía `ir.rule`) incluyen la propia carpeta, pero `_check_can_manage_self` (escritura sobre la carpeta en sí) exige grupo o empleado en el **padre**, no en la propia carpeta — son dos comprobaciones distintas a propósito, no un descuido. Si se toca una sin la otra se rompe la regla de negocio "el permiso da acceso al contenido, no al continente".
